                          ...
                          CtrlSystem *system)
{
    ...
    for (node = system->targets[X_SCREEN_TARGET]; node; node = node->next) {
        ...
        screen_name = g_strdup_printf("X屏幕%d",
        ...
        if (child) {
            ...
            add_page(child, help, ctk_window, &iter, NULL, "OpenGL设置",
                     NULL, NULL, NULL);
        }
        ...
        if (child) {
            ...
            add_page(child, help, ctk_window, &iter, NULL,
                     "图形信息", NULL, ctk_glx_probe_info, NULL);
        }
        ...
        if (child) {
            ...
            add_page(child, help, ctk_window, &iter, NULL, "VDPAU信息",
                     NULL, NULL, NULL);
        }
    ...
    for (node = system->targets[GPU_TARGET]; node; node = node->next) {
        ...
        if (child) {
            ...
            add_page(child, help, ctk_window, &iter, NULL, "温度设置",
    ...
    for (node = system->targets[GPU_TARGET]; node; node = node->next) {
        ...
        if (widget) {
            ...
            add_page(widget, help, ctk_window, NULL, NULL, "管理许可证",
    ...
    add_page(GTK_WIDGET(ctk_window->ctk_config),
             ...
             ctk_window, NULL, NULL, "nvidia-settings配置",
             NULL, NULL, NULL);
    ...
    gtk_window_set_title(GTK_WINDOW(object), "NVIDIA X服务器设置");
    ...
    label = gtk_label_new("XXXXXX服务器显示配置XXXX");
...
static GtkWidget *create_quit_dialog(CtkWindow *ctk_window)
{
    ...
    dialog = gtk_dialog_new_with_buttons("确定退出？",
